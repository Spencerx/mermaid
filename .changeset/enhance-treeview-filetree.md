...
- **Built-in icons**: Files and directories get a default `file`/`folder` icon
- **Icon overrides**: `icon(pack:name)` syntax to use any icon from a registered iconify pack (via `registerIconPacks`); `icon(none)` hides the icon
